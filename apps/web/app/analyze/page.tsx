...
import type { AnalysisBundle, PredictiveResult } from "@/lib/types";
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="container flex items-center justify-between h-14">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs text-muted-foreground hidden sm:block">
              {dataset.name} · target: <strong>{target}</strong> ·{" "}
      ...
      <div className="container py-6 max-w-6xl">
        ...
        {activeTab === "interventions" && (
          <InterventionsTab interventions={analysis.interventions} target={target} />
        )}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <MetricCard
          label="Best Model R²"
          value={best.metrics.r2.toFixed(3)}
        ...
        <MetricCard
          label="Causal Levers"
          value={String(analysis.causal.filter((e) => e.p_value < 0.05).length)}
          sub="p < 0.05"
          color="blue"
        />
        <MetricCard
          label="Recommendations"
          value={String(analysis.interventions.length)}
      ...
      <div className="rounded-xl border border-border/60 bg-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border/60">
          ...
          <p className="text-xs text-muted-foreground">
            All models trained on the same 80/20 split. Best by test R² is selected.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {analysis.predictive.map((r) => (
                <tr
                  key={r.model}
                  className={`border-b border-border/20 ${r.is_winner ? "bg-primary/5" : ""}`}
                >
